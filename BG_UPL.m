function [UPL,mUPL] = BG_UPL(DataP,DataO)
%%% 
%This function calculate the unchanneled path length of the bare ground.
% Input: 
% DataP is vegetation biomass matrix 
% DataO is water content matrix
% Output:
% UPL: the unchanneled path length of every bare ground point
% mUPL: the mean of UPL

[M1,N1] = size(DataP) ;
dx = 0.5 ; % grid size
% Threshold = 0.05 ;
% BW = imbinarize(rescale(DataP),Threshold) ; % Divide DataP into vegetation (1) and bare ground (0)
BW = imbinarize(DataP,'adaptive') ;
...
Abandon_column_index = [1:Abandon_length,N1+1-Abandon_length:N1] ;
%
UPL = zeros(length(Index1),1) ;
LabelNum = 1e6 ; % A label represents the lowest point and water cannot reach the vegetation patch
LabelNum1 = 1e7 ; % A label represents the lowest point and water reach the boundary
parfor ii = 1:length(Index1)
    if (ismember(Index1(ii),Abandon_row_index) == 1) || (ismember(Index2(ii),Abandon_column_index) == 1)
        ...
        while (State_N(IndX,IndY) == 0) && (State_S(IndX,IndY) == 0) && (State_E(IndX,IndY) == 0)  && (State_W(IndX,IndY) == 0) && (State_NE(IndX,IndY) == 0) && (State_NW(IndX,IndY) == 0) && (State_SE(IndX,IndY) == 0) && (State_SW(IndX,IndY) == 0)
            ...
            if sum(double(Slope < 0)) == 8 % lowest point
                ...
                break
            elseif (ismember(IndX,Abandon_row_index) == 1) || (ismember(IndY,Abandon_column_index) == 1)
                n1 = LabelNum1/dx/2 ; n2 = LabelNum1/(sqrt(2)*dx)/2 ; % just a label to present that surface water reach the boundary
                break
            else
                [~,Position] = max(Slope) ;
                if Position == 1
...
UPL(UPL == LabelNum) = nan ; UPL(UPL == LabelNum1) = nan ;
